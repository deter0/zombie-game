...
function Wind:Start()
	...
	local function AddHash(Instance)
		Instance:SetAttribute("Hash", math.random(1, 100000));
	end
	for _, Instance in ipairs(CollectionService:GetTagged(Tag)) do AddHash(Instance); end;
	CollectionService:GetInstanceAddedSignal(Tag):Connect(AddHash);
	...
	RunService.Heartbeat:Connect(function()
		...
		for index, Part in ipairs(PartList) do
			...
			local WindNoise2 = rad(noise(Part.Position.X, now+Part.Position.X-Part:GetAttribute("Hash"), Part.Position.Z));
			local WindNoise3 = rad(noise(Part.Position.X, now+Part.Position.Z-Part:GetAttribute("Hash"), Part.Position.Z));
